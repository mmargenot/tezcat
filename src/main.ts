...
import { RemembranceView, VIEW_TYPE } from './search_view';
...
interface RemembranceAgentSettings {
...
const DEFAULT_SETTINGS: RemembranceAgentSettings = {
...
export default class RemembranceAgent extends Plugin {
    settings: RemembranceAgentSettings;
    ...
    private get CONTEXT_WINDOW_WORDS() { 
        return Math.ceil(this.settings.chunkSize / 0.75); // Dynamic calculation based on chunk size
    }
    ...
    async onload() {
        ...
        this.registerCommand('rebuild-vector-database', 'Rebuild Vector Database', () => this.rebuildVectorDatabase());
        this.registerCommand('process-outdated-notes', 'Process Outdated Notes', () => this.processOutdatedNotes());
        ...
        try {
            ...
            for (const file of files) {
                try {
                    ...
                    const result = await this.databaseService.upsertNote(
                        file.path, 
                        file.name, 
                        file.basename, 
                        content
                    ...
                    if (result.changed) {
                        // Note was created or updated - process vectors
                        await this.databaseService.processNoteForVectors(
                            result.noteId, 
                    ...
                    // Show progress every 10 files (only when showing notices)
                    if (showNotice && processed % 10 === 0) {
    ...
    async rebuildVectorDatabase() {
        if (!this.ensureSystemValid()) {
            return;
        }
        
        new Notice('Starting vector database rebuild...');
        
        try {
            await this.databaseService.rebuildVectorDatabase(
                this.chunkingService,
                this.embeddingService,
                (processed, total) => {
                    if (processed % 10 === 0 || processed === total) {
                        new Notice(`Processed ${processed}/${total} notes`, 2000);
                    }
                }
            );
            ...
            new Notice('Vector database rebuild completed successfully!');
        } catch (error) {
            logger.error('Plugin', 'Vector database rebuild failed', error);
            new Notice('Vector database rebuild failed. Check console for details.');
        }
    }

    async processOutdatedNotes() {
        if (!this.ensureSystemValid()) {
            return;
        }
        
        new Notice('Processing outdated notes...');
        
        try {
            await this.databaseService.processAllOutdatedNotes(
                this.chunkingService,
                this.embeddingService,
                (processed, total) => {
                    new Notice(`Processed ${processed}/${total} outdated notes`);
                }
            );
            new Notice('Outdated notes processing completed!');
        } catch (error) {
            logger.error('Plugin', 'Processing outdated notes failed', error);
            new Notice('Processing outdated notes failed. Check console for details.');
        }
    }
    ...
    async performAutomaticSetup() {
        ...
        try {
            ...
            // Automatically build vector index if it doesn't exist
            logger.info('Plugin', 'Checking vector index...');
...
• Chunk Vectors: ${stats.vectorsByType.chunk}
...
• Orphaned Chunks: ${stats.orphanedData.orphanedChunks}`;
            ...
            logger.info('Plugin', `Vector Database Stats: totalNotes=${stats.totalNotes}, processedNotes=${stats.processedNotes}, totalVectors=${stats.totalVectors}, noteVectors=${stats.vectorsByType.note}, chunkVectors=${stats.vectorsByType.chunk}`);
        } catch (error) {
    ...
    private onActiveLeafChange(leaf: WorkspaceLeaf | null) {
        ...
        if (leaf?.view instanceof MarkdownView) {
            ...
            if (leaves.length > 0) {
                const searchView = leaves[0].view as RemembranceView;
    ...
    private async updateSearchPanel(results: SearchResult[]) {
        const leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE);
        if (leaves.length > 0) {
            const view = leaves[0].view as RemembranceView;
    ...
    private async handleFileModify(file: any) {
        ...
        try {
            ...
            // Use upsertNote to check for actual changes
            const result = await this.databaseService.upsertNote(file.path, file.name, file.basename, content);
            
            // Only reprocess if changes were detected
            if (result.changed) {
                logger.debug('Plugin', `Reprocessing modified note: ${file.path}`);
                await this.databaseService.reprocessSingleNote(result.noteId, this.chunkingService, this.embeddingService);
            }
        } catch (error) {
    ...
    private async handleFileCreate(file: any) {
        ...
        try {
            ...
            // Use upsertNote to handle both new files and existing files during vault initialization
            const result = await this.databaseService.upsertNote(file.path, file.name, file.basename, content);
            
            // Only reprocess if changes were made
            if (result.changed) {
                logger.debug('Plugin', `Reprocessing note due to changes: ${file.path}`);
                await this.databaseService.reprocessSingleNote(result.noteId, this.chunkingService, this.embeddingService);
            }
        } catch (error) {
    ...
    private async handleNoteNavigation(leaf: WorkspaceLeaf | null) {
        ...
        try {
            ...
            // If we're navigating to a different note, reprocess the previous note if it changed
            if (this.currentNoteId && newNoteId !== this.currentNoteId) {
                logger.debug('Plugin', `Note navigation detected, checking previous note for changes`);
                await this.databaseService.reprocessSingleNote(this.currentNoteId, this.chunkingService, this.embeddingService);
            }
...
class TezcatSettingTab extends PluginSettingTab {
    plugin: RemembranceAgent;
    private pendingSettings: RemembranceAgentSettings;
    private hasSensitiveChanges: boolean = false;

    constructor(app: App, plugin: RemembranceAgent) {
        super(app, plugin);
        this.plugin = plugin;
    ...
    private async saveAndReindex() {
        try {
            ...
            await this.plugin.databaseService.rebuildVectorDatabase(
                this.plugin.chunkingService, 
                this.plugin.embeddingService,
                (processed: number, total: number) => {
                    processedFiles = processed;
                    if (processed % 10 === 0 || processed === total) {
                        new Notice(`Processing files: ${processed}/${total}`, 2000);
                        logger.info('Plugin', `Rebuilt vectors for ${processed}/${total} files`);
                    }
                }
            );
            ...
            logger.info('Plugin', `Reindexing completed: processed ${processedFiles} files`);
...
class SystemStatusModal extends Modal {
    ...
    private plugin: RemembranceAgent;

    constructor(app: App, validationResult: SystemValidationResult, isSystemValid: boolean, plugin: RemembranceAgent) {
        super(app);
    ...
    onOpen() {
        ...
        // Overall status
        const overallEl = contentEl.createDiv('system-status-overall');
        overallEl.style.padding = '12px';
        overallEl.style.marginBottom = '16px';
        overallEl.style.borderRadius = '6px';
        
        if (this.isSystemValid) {
            overallEl.style.background = 'rgba(0, 255, 0, 0.1)';
            overallEl.style.border = '1px solid rgba(0, 255, 0, 0.3)';
            overallEl.innerHTML = '<strong>System Ready</strong><br/>All components are working correctly.';
        } else {
            overallEl.style.background = 'rgba(255, 255, 0, 0.1)';
            overallEl.style.border = '1px solid rgba(255, 255, 0, 0.3)';
            overallEl.innerHTML = '<strong>Setup Required</strong><br/>Some components need attention.';
        }
    ...
    private createStatusItem(container: HTMLElement, title: string, result: ValidationResult) {
        const itemEl = container.createDiv('status-item');
        itemEl.style.padding = '8px 12px';
        itemEl.style.marginBottom = '8px';
        itemEl.style.borderRadius = '4px';
        itemEl.style.border = '1px solid var(--background-modifier-border)';
        
        const headerEl = itemEl.createDiv('status-item-header');
        ...
        if (result.success) {
            ...
            statusEl.innerHTML = 'Issue';
            statusEl.style.color = 'var(--text-error)';
        }
        
        const messageEl = itemEl.createDiv('status-item-message');
    ...
    onOpen() {
        ...
        this.results.forEach((result, index) => {
            ...
            if (result.type === 'chunk') {
                ...
                metaEl.innerHTML = `<span style="background: var(--background-modifier-border); padding: 2px 6px; border-radius: 3px; font-size: 0.8em;">${result.type}</span> <span style="color: var(--text-muted); font-size: 0.9em;">Score: ${result.score.toFixed(3)}</span>`;
