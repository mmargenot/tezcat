...
import { LSHConfig, LSHHashFunction, VectorUtils } from './embedding_service';
...
export class SqlJsDatabaseAdapter implements DatabaseAdapter {
    ...
    async initialize(): Promise<void> {
        try {
            ...
            const wasmPath = '.obsidian/plugins/tezcat/sql-wasm.wasm';
    ...
    async createVectorsTable(): Promise<void> {
        ...
        await this.execute(`
            CREATE TABLE IF NOT EXISTS vectors (
                ...
                note_id TEXT NOT NULL,
                chunk_id TEXT,
                type TEXT NOT NULL CHECK (type IN ('note', 'chunk')),
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                vector BLOB,
                FOREIGN KEY (note_id) REFERENCES notes(id) ON DELETE CASCADE,
    ...
    async save(): Promise<void> {
        ...
        const dbPath = '.obsidian/plugins/tezcat/tezcat.db';
...
export class DatabaseService {
    ...
    async createVector(vector: Omit<Vector, 'id' | 'created_at' | 'updated_at'>): Promise<string> {
        ...
        await this.adapter.execute(`
            INSERT INTO vectors (id, note_id, chunk_id, type, created_at, updated_at, vector)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `, [id, vector.note_id, vector.chunk_id || null, vector.type, now, now, vector.vector]);
    ...
    async getVectorCountByType(): Promise<{ note: number; chunk: number }> {
        ...
        const counts = { note: 0, chunk: 0 };
        results.forEach(row => {
            if (row.type === 'note') counts.note = row.count;
            if (row.type === 'chunk') counts.chunk = row.count;
        });
    ...
    // Vector Processing Orchestration Methods
    async processNoteForVectors(noteId: string, chunkingService: any, embeddingService: any): Promise<void> {
        // Get the note
        ...
        try {
            // Clean up existing vectors for this note
            await this.deleteVectorsForNote(noteId);
            await this.deleteChunksForNote(noteId);

            // Generate note-level vector
            ...
            // Generate chunks and chunk vectors
            const chunks = await chunkingService.chunkText(note.text);
            
            if (chunks.length > 0) {
                // Create chunk records first
                const chunkIds = [];
                for (let i = 0; i < chunks.length; i++) {
                    const chunk = chunks[i];
                    const chunkId = await this.createChunk({
                        note_id: noteId,
                        chunk_index: i,
                        text: chunk.text
                    });
                ...
                const chunkVectors = await embeddingService.embedTexts(chunkTexts);
                ...
                for (let i = 0; i < chunkVectors.length; i++) {
                    await this.createVector({
                        note_id: noteId,
                        chunk_id: chunkIds[i],
                        type: VectorType.CHUNK,
                        vector: chunkVectors[i]
                    });
                }
            }

            this.logger.debug('SqlJsAdapter', `Successfully processed note ${noteId} with ${chunks.length} chunks`);
        } catch (error) {
    ...
    /**
     * Reprocess a single note only if it has changed since last vector generation
     * This is a wrapper around the cleanup and processing operations for convenience
     */
    async reprocessSingleNote(noteId: string, chunkingService: any, embeddingService: any): Promise<void> {
        try {
            // Get the note to verify it exists first
            const note = await this.getNote(noteId);
            if (!note) {
                this.logger.debug('SqlJsAdapter', `Note ${noteId} not found, skipping reprocessing (note may have been deleted)`);
                return;
            }

            // Check if processing is needed
            if (await this.isNoteVectorUpToDate(noteId)) {
                this.logger.debug('SqlJsAdapter', `Note ${noteId} vectors are up to date, skipping reprocessing`);
                return; // Already up to date, no need to reprocess
            }

            this.logger.debug('SqlJsAdapter', `Reprocessing note ${noteId} due to changes`);
            ...
            // Clean up existing vectors and chunks for this note
            await this.deleteVectorsForNote(noteId);
            await this.deleteChunksForNote(noteId);

            // Reprocess the note with new vectors
            await this.processNoteForVectors(noteId, chunkingService, embeddingService);
            
            this.logger.info('SqlJsAdapter', `Successfully reprocessed note ${noteId} (${note.path})`);
        } catch (error) {
            // Handle specific case where note was deleted between checks
            if (error instanceof Error && error.message.includes('Note not found')) {
                this.logger.debug('SqlJsAdapter', `Note ${noteId} was deleted during reprocessing, skipping`);
                return;
            }
            
            // Don't throw errors to avoid crashing the UI - just log them
            this.logger.error('SqlJsAdapter', `Failed to reprocess note ${noteId}`, error);
        }
    }
    ...
    async processAllOutdatedNotes(chunkingService: any, embeddingService: any, progressCallback?: (processed: number, total: number) => void): Promise<void> {
        // Get all notes that need processing
        const outdatedFiles = await this.getOutdatedFiles();
        
        this.logger.info('SqlJsAdapter', `Processing ${outdatedFiles.length} outdated notes`);
        
        for (let i = 0; i < outdatedFiles.length; i++) {
            const file = outdatedFiles[i];
            
            try {
                await this.processNoteForVectors(file.noteId, chunkingService, embeddingService);
                
                // Report progress
                if (progressCallback) {
                    progressCallback(i + 1, outdatedFiles.length);
                }
            } catch (error) {
                this.logger.error('SqlJsAdapter', `Failed to process note ${file.noteId} (${file.path})`, error);
                // Continue with other notes even if one fails
            }
        }
        
        this.logger.info('SqlJsAdapter', `Completed processing ${outdatedFiles.length} notes`);
    }

    async rebuildVectorDatabase(chunkingService: any, embeddingService: any, progressCallback?: (processed: number, total: number) => void): Promise<void> {
        this.logger.info('SqlJsAdapter', 'Starting vector database rebuild...');
        ...
        try {
            ...
            this.logger.info('SqlJsAdapter', `Rebuilding vectors for ${allNotes.length} notes`);
            
            for (let i = 0; i < allNotes.length; i++) {
                const note = allNotes[i];
                
                try {
                    // Force reprocessing by deleting existing vectors first
                    await this.deleteVectorsForNote(note.id);
                    await this.deleteChunksForNote(note.id);
                    
                    await this.processNoteForVectors(note.id, chunkingService, embeddingService);
                    
                    // Report progress
                    if (progressCallback) {
                        progressCallback(i + 1, allNotes.length);
                    }
                } catch (error) {
                    this.logger.error('SqlJsAdapter', `Failed to rebuild vectors for note ${note.id} (${note.path})`, error);
                    // Continue with other notes even if one fails
                }
            }
            
            this.logger.info('SqlJsAdapter', `Completed vector database rebuild for ${allNotes.length} notes`);
        } catch (error) {
            this.logger.error('SqlJsAdapter', 'Vector database rebuild failed', error);
            throw error;
        }
    }
    ...
    async getVectorDatabaseStats(): Promise<{
        ...
        totalVectors: number;
        vectorsByType: { note: number; chunk: number };
