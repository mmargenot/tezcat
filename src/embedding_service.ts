...
export class OllamaEmbeddingProvider extends EmbeddingProvider {
    ...
    async embed_one(text: string): Promise<Int8Array> {
        ...
        if (!response.ok) {
            ...
            this.logger.error('OllamaProvider', `Error response: ${errorText}`);
    ...
    async embed_many(texts: string[]): Promise<Int8Array[]> {
        ...
        for (let i = 0; i < texts.length; i += batchSize) {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                this.logger.error('OllamaProvider', `Batch error response: ${errorText}`);
                throw new Error(`Ollama API error: ${response.status} ${response.statusText} - ${errorText}`);
            }
    ...
    async validate(): Promise<void> {
        try {
            ...
            const modelExists = data.models.some((model: any) => model.name === this.modelName);
            if (!modelExists) {
    ...
    async embed_many(texts: string[]): Promise<Int8Array[]> {
        ...
        for (let i = 0; i < texts.length; ) {
            ...
            const batchEmbeddings = await this.makeRequestWithRetry(async () => {
                ...
                if (!response.ok) {
                    const errorText = await response.text();
                    this.logger.error('OpenAIProvider', `Batch error response: ${errorText}`);
                    throw new Error(`OpenAI API error: ${response.status} ${response.statusText} - ${errorText}`);
                }
